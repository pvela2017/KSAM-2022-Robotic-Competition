...
class Predictor(object):
    ...
    def visual(self, output, img_info, cls_conf=0.35):
        ...
        for i in range(len(output)):
        	if output[i, 6] == output[i,6] == diseased_cls:
...
def image_capture_demo(predictor, vis_folder, current_time, args):
    ...
    if args.save_result:
        ...
        save_folder = os.path.join(
            usb_path, "Fruit Counting"
        )
        os.makedirs(save_folder, exist_ok=True)
		save_path = os.path.join(save_folder, "Counting.txt")
        logger.info(f"Text save_path is {save_path}")
    ...
    while True:
        ...
        if left_grabbed and right_grabbed:
            ...
            if True: 
            	left_outputs, left_img_info = predictor.inference(left_frmae)
            	number_of_normal_left, number_of_diseased_left, result_frame_left = predictor.visual(left_outputs[0], img_info, predictor.confthre)
            	right_outputs, right_img_info = predictor.inference(right_frame)
            	number_of_normal_right, number_of_diseased_right, result_frame_right = predictor.visual(right_outputs[0], img_info, predictor.confthre)
            	cv2.imwrite(save_folder + "left_image.jpg", left_frame)
            	cv2.imwrite(save_folder + "right_image.jpg", right_frame)

            	if args.save_result:
            		f=open(save_path + "counting.txt", 'a')
            		f.write("normal: {0} diseased: {1}\n".format(number_of_normal, number_of_diseased))
            		f.close()
            	else:
            		cv2.namedWindow("yolox", cv2.WINDOW_NORMAL)
            		cv2.imshow("yolox", result_frame)
